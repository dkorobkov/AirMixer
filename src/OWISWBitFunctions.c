...
//    __delay_cycles(OWI_DELAY_H_STD_MODE);
	_delay_ms(1); // Длина ресета не менее 480 мкс
}
...
cli();//    __disable_interrupt();
    ...
    _delay_us(_OWI_DELAY_A_STD_MODE);//__delay_cycles(OWI_DELAY_A_STD_MODE);
    
    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    _delay_us(_OWI_DELAY_B_STD_MODE);//__delay_cycles(OWI_DELAY_B_STD_MODE);
    
    // Restore interrupts.
    sei();//__restore_interrupt(intState);
}
...
cli();//    __disable_interrupt();
    ...
    // Drive bus low and delay.
    OWI_PULL_BUS_LOW(pins);
    _delay_us(_OWI_DELAY_C_STD_MODE);//__delay_cycles(OWI_DELAY_C_STD_MODE);
    
    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    _delay_us(_OWI_DELAY_D_STD_MODE);//__delay_cycles(OWI_DELAY_D_STD_MODE);
...
cli();//    __disable_interrupt();
    ...
    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    _delay_us(_OWI_DELAY_E_STD_MODE);//__delay_cycles(OWI_DELAY_E_STD_MODE);
    
    // Sample bus and delay.
    bitsRead = OWI_PIN & pins;
    _delay_us(_OWI_DELAY_F_STD_MODE);//__delay_cycles(OWI_DELAY_F_STD_MODE);
...
//    intState = __save_interrupt();
    ...
    // Drive bus low and delay.
    OWI_PULL_BUS_LOW(pins);
    _delay_us(_OWI_DELAY_H_STD_MODE);//__delay_cycles(OWI_DELAY_H_STD_MODE);
    
    // Release bus and delay.
    OWI_RELEASE_BUS(pins);
    _delay_us(_OWI_DELAY_I_STD_MODE);//__delay_cycles(OWI_DELAY_I_STD_MODE);
    
    // Sample bus to detect presence signal and delay.
    presenceDetected = ((~OWI_PIN) & pins);
    _delay_us(_OWI_DELAY_J_STD_MODE);//__delay_cycles(OWI_DELAY_J_STD_MODE);
